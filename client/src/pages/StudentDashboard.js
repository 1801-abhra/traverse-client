...
function StudentDashboard() {
  ...
  useEffect(() => {
    ...
    socket.on('ride:accepted', (ride) => {
      ...
      setSharedRides(prev => prev.filter(r => r._id?.toString() !== rideId?.toString()));
  ...
  // Recheck driver availability when student comes back to app
  useEffect(() => {
    const handleVisibilityChange = () => {
  ...
  const fetchActiveRide = async () => {
    try {
      ...
      if (res.data) {
        ...
        // Only clear if no completed ride waiting for rating
        if (!completedRide) {
          setActiveRide(null);
        }
      }
